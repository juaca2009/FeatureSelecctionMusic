...
class conversor:
    ...
    def analizarOutlers(self):
        columnasGaussianas = self.__outlers['G'].keys()
        columnasNoGaussianas = self.__outlers['NG'].keys()
        for i in columnasGaussianas:
            valorTemp = self.__dataFrame[i].values
            valorTemp = valorTemp[0]
            if valorTemp < self.__outlers['G'][i][0]:
                self.__dataFrame[i] = self.__outlers['G'][i][0]
            elif valorTemp > self.__outlers['G'][i][1]:
                self.__dataFrame[i] = self.__outlers['G'][i][1]
        for i in columnasNoGaussianas:
            valorTemp = self.__dataFrame[i].values
            valorTemp = valorTemp[0]
            if valorTemp < self.__outlers['NG'][i][0]:
                self.__dataFrame[i] = self.__outlers['NG'][i][2]
            elif valorTemp > self.__outlers['NG'][i][1]:
                self.__dataFrame[i] = self.__outlers['NG'][i][2]

    def escalarValores(self):
        columnasGaussianas = self.__escalar['G'].keys()
        columnasNoGaussianas = self.__escalar['NG'].keys()
        for i in columnasGaussianas:
            valorTemp = self.__dataFrame[i].values
            valorTemp = valorTemp[0]
            self.__dataFrame[i] = (valorTemp-self.__escalar['G'][i][0])/self.__escalar['G'][i][1]
        for i in columnasNoGaussianas:
            valorTemp = self.__dataFrame[i].values
            valorTemp = valorTemp[0]
            self.__dataFrame[i] = (valorTemp-self.__escalar['NG'][i][0])/(self.__escalar['NG'][i][1]-self.__escalar['NG'][i][0])

    def transformarEntrada(self, _dataFrame):
        self.setDataFrame(_dataFrame)
        self.analizarOutlers()
        self.escalarValores()
        
    def setDataFrame(self, _dataFrame):
        self.__dataFrame = _dataFrame

    def getDataFarame(self):
        return self.__dataFrame

if __name__ == '__main__':
    columnas = ['flt', 'lds', 'alds', 'strpk', 'nrg', 'flu', 'entr', 'danc', 'bpm',
                'ptch', 'mfcc1', 'mfcc2', 'mfcc3', 'mfcc4']
    datosPrueba = [[0.240082, -14.933464,  0.897602,  1.146760,  0.022006,  0.071873,  7.002771,  1.288145,  114.862709, 
                    0.419456, -723.15356,  104.381910,  13.517786,  49.038660]]
    df = pd.DataFrame(datosPrueba, columns=columnas)
    print(df)
    a = conversor()
    a.transformarEntrada(df)
    df = a.getDataFarame()
    print(df)
    print(df[['entr', 'danc']])
